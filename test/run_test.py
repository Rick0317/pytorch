...
def get_selected_tests(options):
    ...
    if options.shard:
        ...
        if test_config not in test_file_times:
            ...
            selected_tests = shards[which_shard - 1]

    # skip all distributed tests if distributed package is not available.
    if not dist.is_available():
        selected_tests = selected_tests.exclude_tests(
            DISTRIBUTED_TESTS, "PyTorch is built without distributed support.")

    # skip tests that require LAPACK when it's not available
    if not torch._C.has_lapack:
        selected_tests = selected_tests(TESTS_REQUIRING_LAPACK, "PyTorch is built without LAPACK support.")
